...
| 4 | 資料合約已具備：`vite.config.ts` 單一 plugin 於 dev 將 `../api/**` 服務為 `/api/*`、build 自動 copy 進 `dist/api/`；前端 runtime 讀 `api/index.json`（categories[]）→ 依側欄 lazy 載入 `api/items/{g}.json?v={crawled_at}`（契約 v2，每筆 history ≤2 點） | P2（沿用） | `web/vite.config.ts` | 003 載入層（`useItems`）直接消費，無需改動 build 合約 |
...
2. **資料層**：`useItems` 依 §2.4 實作錯誤分類（fetch／parse）與 `isStale`（>7 天，與 007 共用）；runtime 讀 `api/index.json`（categories[]＋crawled_at）→ 依側欄 lazy 載入 `api/items/{g}.json?v={crawled_at}`（契約 v2）；全站搜尋/詳情定位用 loadAll 聚合；快取以 `?v={crawled_at}` 強制取新，詳見 002 §1.7。
...
- [ ] 卡片顯示：名稱、spec chips、目前價（NT$）、昨日漲跌、sparkline、價格年齡
- [ ] 漲跌：漲紅／跌綠／持平灰，附文字；缺昨日價顯示「—」
- [ ] sparkline < 2 筆顯示「資料不足」（不畫線）；折線右端有圓點、下方有日期範圍標籤
- [ ] 價格年齡標籤：今天=藍色「今天」、1天前=「昨天」、2~7天=「N天前」、>7天=「M/D」（淡灰）；hover tooltip 顯示完整日期
