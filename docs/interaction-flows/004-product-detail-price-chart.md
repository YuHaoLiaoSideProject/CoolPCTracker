...
| **前置條件** | ① 同 origin 的資料 API（`api/index.json` → `api/items/YYYYMMDD[_n].json`）已可存取 ② 該商品存在於載入資料且具備 `history` 陣列 ③ 訪客瀏覽器支援 JavaScript（圖表以 lightweight-charts 渲染） |
...
| **系統回應** | 路由切換至詳情頁，URL 帶入該商品 id（如 `#/product/3f9a1c2b8e4d5f6a`）；頁面顯示載入中狀態（skeleton／spinner），背景重新 fetch 同 origin 的資料 API（`api/index.json` → `latest_file` → `api/items/YYYYMMDD[_n].json`） |
...
| **操作前** | 詳情頁顯示載入中 |
| **系統回應** | fetch 成功 → 依 URL 中的 id 於載入資料的 items 陣列定位商品；找到 → 取得商品物件（name / spec / status / history…）；找不到 → 切換錯誤畫面 |
...
| **系統回應** | 以 lightweight-charts 將 `history` 的 `[d, p]` 時間序列渲染為折線圖：X 軸為日期、Y 軸為價格；支援內建縮放／平移與自寫 tooltip（懸停顯示日期與價格）；history 僅一筆時顯示單點圖 |
...
| 項目 | 限制說明 |
|------|----------|
| **資料來源** | 僅讀取同 origin 的資料 API（`api/index.json` → `api/items/YYYYMMDD[_n].json`，靜態檔案），無後端 API、無即時請求 |
| **資料新鮮度** | 資料由 GitHub Actions 每日爬蟲更新（約台北 14:00），詳情頁顯示 meta.crawled_at 對應的最後更新時間；可能與原價屋現場價格有時間差 |
| **歷史取樣** | 歷史採「每日一點累積」策略：每次成功爬取（含平價日）append 當日價格點，跨日連續每日有值；失敗未爬取日如實留白不補點 |
...
- [ ] lightweight-charts 趨勢圖正確渲染 `[d, p]` 時間序列（X=日期、Y=價格）
