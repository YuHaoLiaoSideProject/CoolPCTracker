...
| 迷你趨勢 | 商品最近 7 日歷史價格的 sparkline（來源：`data/items.json` 的 history） |
...
| **操作前** | 商品資料（`data/items.json`）已載入完成，頁面顯示商品清單 |
...
| 3 | 追蹤商品已下架（不再出現在當日 `items.json`） | 追蹤清單中該商品標示「已下架」，價格欄顯示「—」 | 使用者可手動移除；不自動清除 |
| 4 | 商品資料（`items.json`）載入失敗 | 追蹤/比價頁面顯示「資料載入失敗」與「重新載入」按鈕 | 點「重新載入」重試；既有 localStorage 追蹤資料不受影響 |
